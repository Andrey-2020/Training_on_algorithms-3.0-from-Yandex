...
### [Решение](Тупики/TaskA18.py)

## 18. Дек с защитой от ошибок - Б

Можно было бы используя Класс deque() модуля collections решить намного проще, но пытался реализовать что-то креативное.
В один список помещались элементы, которые должны быть в начале дека. Во второй список те, что предназначались для конца. В списке начала дека элементы расположены в обратном порядке из-за чего удаление с конца будет происходить за линейное время.
...
### [Решение](Дек_с_защитой_от_ошибок/TaskB18.py)

## 19. Коммерческий калькулятор - А

Задача легко решается с использованием кучи. Суммируем 2 вершины кучи минимумов. Результат вставляем в кучу и просеиваем.
...
### [Решение](Коммерческий_калькулятор/Task19A.py)

## 19. Хипуй - Б

Организовать структуру Heap.
Реализация добавления элемента: добавляем элемент в конец списка (на первое свободное место) и просеиваем вверх до тех пор пока он либо не станет со своим отцом стоять в правильном порядке, либо не дойдем до начала.
Индекс предка относительно позиции элемента(pos): (pos - 1 )//2

Реализация удаления элемента: корень сохраняем в переменную которую вернем, копируем последний элемент в корень и будем просеивать вниз. На некоторое время оставим последний элемент для того, чтобы уменьшить количество случаев. Копию последнего значения в корне будем просеивать вниз, это значение и последнее никогда не свапнутся из-за строгого неравенства.
...
### [Решение](Хипуй/Task19B.py)

## 20. Машинки - А

В случае, когда на полу заканчивается место, нужно понять, какую машинку удалить с полки из тех, которые стоят на полу. Подходит жадный алгоритм. Мы будем заменять машинку на полу машинкой на полке согласно следующему принципу: выбираем машинку, которая использовалась последней (с наибольшим номером в последовательности). Можно доказать жадность этого алгоритма методом от противного: предположим, что мы удалим другую машинку, а не эту. Оказывается, что наше решение не изменится или ухудшится.

Реализация: для каждого из вхождений машинок в последовательности длиной P определить, когда она будет использована в следующий раз. Для каждой позиции за линейный проход по последовательности в обратном порядке определим, когда в следующий раз на какой позиции будет использовано такое же число (такая же машина). Можно реализовать с помощью словаря (запоминаем, когда последний раз встречался элемент в последовательности и когда встретим его запоминаем это число, заменяем номер вхождения).

Используя кучу максимумов (кучу пар: время использования, номер машинки) и поддерживая словарь используемых машинок на полу, можно каждый раз убирать с пола те машинки, которые используются позже всего. Если данная машинка уже есть, то добавляем элемент в кучу, не изменяя словарь используемых машинок.
...
### [Решение](Машинки/Task20A.py)

## 20. Пирамидальная сортировка - Б

Пирамидальная сортировка. Cоздание двоичной кучи и многократное извлечения максимального элемента из кучи, который затем помещается в конец отсортированной части массива.
...
### [Решение](Пирамидальная_сортировка/Task20B.py)

## 21. Разложение в сумму кубов - А

Задача на динамическое программирование. Решается переформулированием задачи в виде индуктивной последовательности подзадач, от крайнего случая к более сложным. Перебираем числа x: от 2 до нужного N, вычисляя наименьшее число слагаемых разложения по кубам и кешируем это значение. На каждом шаге сравниваем число слагаемых для набора кубов cubes числа x, используя ранее рассчитанное значение dp[x - cube].
...
### [Решение](Разложение_в_сумму_кубов/Task21A.py)

## 21. Три единицы подряд - Б

Задача на динамическое программирование. Фактически это обобщенные числа фибоначи, формула выглядит так: dp[i] = dp[i - 1] + dp[i - 2] + dp[i - 3]. В задачах на динамическое программирование самое сложное придумать формулу.
...
### [Решение](Три_единицы_подряд/Task21B.py)